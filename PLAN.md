...
- **2B Read editor context**：扩展读活动编辑器/选区 → 经 dsh workspace/settings 通道注入会话上下文（先验证 DSH 是否有注入点，再定实现）。
...
- **Phase 2 入向**（2B）：扩展读编辑器状态 → DSH 注入点。
...
- **实现期再定**：`/api` 健康端点确切路径；工作区预置注入点（2B 前提）；WS downlink 消息形态（2A 备选）。
